...
class VMM(nn.Module):
    ...
    def forward_core(self, x: torch.Tensor,
        ...
        K = 4
        # print("hello")
        xs = mair_ids_scan(x, mair_ids[0])  # B, K, d_inner
        # print(f"xs shape:{xs.shape}, sx after reshape:{xs.reshape(B, -1, L).shape} x_proj_weight shape:{self.x_proj_weight.reshape(-1, D, 1).shape}")
...
class WaDAIR(nn.Module):
    ...
    def __init__(self,
        ...
        self.WaDAIR_difscan = WaDAIR_difscan(img_size=img_size, patch_size=patch_size, in_chans=in_chans,
                                         ...
                                         img_range=img_range, upsampler=upsampler, resi_connection=resi_connection,
                                         dynamic_ids=dynamic_ids, scan_len=scan_len, mlp_ratio=mlp_ratio, **kwargs)
    ...
    def forward(self, x):
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            out_wtt = self.conv_after_body(out_wtt_feat) + x_crop
            out = self.conv_after_body(self.alpha * out_norm + (1 - self.alpha) * out_wtt) + x
            ...
            out_wtt_feat = out_wtt_feat[:, :, :min_h, :min_w]
            out_norm = out_norm[:, :, :min_h, :min_w]
            out = self.upsample(self.alpha * out_norm + (1 - self.alpha) * out_wtt_feat)
