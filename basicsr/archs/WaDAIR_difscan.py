...
class WaDAIR_difscan(nn.Module):
    ...
    def __init__(self,
        ...
        super(WaDAIR_difscan, self).__init__()
        num_in_ch = in_chans
        num_out_ch = in_chans
        num_feat = 64
    ...
    def forward_features(self, x):
        ...
        if self.dynamic_ids or (self.image_size != (H, W)):
            ...
            # LH用 Column Scan
            LH_scan_ids, LH_inverse_ids = mair_column_ids_generate((1, 1, H // 2, W // 2), self.scan_len)
            # HH用 Diagonal Scan
            HH_scan_ids, HH_inverse_ids = mair_diagonal_ids_generate((1, 1, H // 2, W // 2), self.scan_len)
            if torch.cuda.is_available():
...
if __name__ == '__main__':
    ...
    net = WaDAIR_difscan(img_size=(320, 180), embed_dim=60, d_state=1, ssm_ratio=1.1, dynamic_ids=False, mlp_ratio=1.6,
               upscale=4).cuda()
