...
try:
    ...
    from shift_scanf_util_wtt import mair_row_ids_generate, mair_column_ids_generate, mair_diagonal_ids_generate, \
        mair_ids_scan, mair_ids_inverse
...
class VMM(nn.Module):
    def __init__(
        ...
        self.x_proj_weight = nn.Parameter(self.x_proj[0].weight)  # (N, inner)
        del self.x_proj
        ...
        self.dt_projs_weight = nn.Parameter(self.dt_projs[0].weight)  # (inner, rank)
        self.dt_projs_bias = nn.Parameter(torch.stack([t.bias for t in self.dt_projs], dim=0))  # (K=4, inner)
        del self.dt_projs

        self.A_logs = self.A_log_init(self.d_state, self.d_inner, copies=1, merge=True)  # (K=41, D, N)
        ...
        self.gating = ShuffleAttn(in_features=self.d_inner, out_features=self.d_inner, group=self.d_inner)
    ...
    def forward_core(self, x: torch.Tensor,
        ...
        Args:
            x: 输入图像张量 (B, C, H, W)
            mair_ids: ([scan_ids], [reverse_ids])
            x_proj_bias: 可选的线性投影 bias
        ...
        # print(f"mair_ids shape:{mair_ids[0].shape}")

        D = self.d_inner
        R = self.dt_rank

        xs = mair_ids_scan(x, mair_ids[0], bkdl=True)
        L = xs.shape[-1]
        # print(f"xs before reshape: {xs.shape},L:{L}")
        x_dbl = F.conv1d(xs.reshape(B, -1, L),
                         self.x_proj_weight.reshape(-1, D, 1),
                         bias=(x_proj_bias.reshape(-1) if x_proj_bias is not None else None),
                         groups=1)
        # print(f"x_dbl before reshape:{x_dbl.shape}")
        x_dbl = x_dbl.reshape(B, 1, -1, L)
        dts, Bs, Cs = torch.split(x_dbl, [R, N, N], dim=2)
        dts = F.conv1d(dts.reshape(B, -1, L), self.dt_projs_weight.reshape(1 * D, -1, 1), groups=1)

        xs = xs.float().view(B, -1, L)
        dts = dts.contiguous().float().view(B, -1, L)  # (b, k * d, l)
        Bs = Bs.float().view(B, 1, -1, L)  # (b, k, d_state, l)
        Cs = Cs.float().view(B, 1, -1, L)  # (b, k, d_state, l)
        ...
        out = mair_ids_inverse(out_y, mair_ids[1], shape=(B, -1, H, W), original_L=H * W)
        return out
...
class MaIR_samescan(nn.Module):
    ...
    def __init__(self,
        ...
        super(MaIR_samescan, self).__init__()
        num_in_ch = in_chans
        num_out_ch = in_chans
        num_feat = 64
        ...
        self.conv_ll = nn.Conv2d(embed_dim // 2, embed_dim // 2, 3, 1, 1)
        if not self.dynamic_ids:
            self._generate_ids((1, 1, img_size_ids[0] // 2, img_size_ids[1] // 2))
        self.att_conv = nn.Conv2d(3 * embed_dim, embed_dim, kernel_size=1)
    ...
    def _generate_ids(self, inp_shape):
        B, C, H, W = inp_shape
        # HL用 Row Scan
        HL_scan_ids, HL_inverse_ids = mair_row_ids_generate((1, 1, H, W), self.scan_len)

        # LH用 Column Scan
        LH_scan_ids, LH_inverse_ids = mair_column_ids_generate((1, 1, H, W), self.scan_len)

        # HH用 Diagonal Scan
        HH_scan_ids, HH_inverse_ids = mair_diagonal_ids_generate((1, 1, H, W), self.scan_len)

        # xs_scan_ids, xs_inverse_ids = mair_ids_generate(inp_shape=(1, 1, H, W), scan_len=self.scan_len)# [B,H,W,C]
        if torch.cuda.is_available():
            self.HL_scan_ids, self.HL_inverse_ids = HL_scan_ids.cuda(), HL_inverse_ids.cuda()
            self.LH_scan_ids, self.LH_inverse_ids = LH_scan_ids.cuda(), LH_inverse_ids.cuda()
            self.HH_scan_ids, self.HH_inverse_ids = HH_scan_ids.cuda(), HH_inverse_ids.cuda()
        else:
            self.HL_scan_ids, self.HL_inverse_ids = HL_scan_ids, HL_inverse_ids
            self.LH_scan_ids, self.LH_inverse_ids = LH_scan_ids, LH_inverse_ids
            self.HH_scan_ids, self.HH_inverse_ids = HH_scan_ids, HH_inverse_ids

        del HL_scan_ids, HL_inverse_ids, LH_scan_ids, LH_inverse_ids, HH_scan_ids, HH_inverse_ids

    def forward_features(self, x):
        B, C, H, W = x.shape
        x_size = (x.shape[2], x.shape[3])
        LL, LH, HL, HH = dwt2d(x, wave='haar')
        # LL_feat = self.conv_ll(LL)

        HL_tokens = self.patch_embed(HL)  # 形状[B, N, C]
        LH_tokens = self.patch_embed(LH)
        HH_tokens = self.patch_embed(HH)
        # x = self.patch_embed(x) # N,L,C
        HL_tokens = self.pos_drop(HL_tokens)
        LH_tokens = self.pos_drop(LH_tokens)
        HH_tokens = self.pos_drop(HH_tokens)
        # x = self.pos_drop(x)

        if self.dynamic_ids or (self.image_size != (H, W)):
            # HL用 Row Scan
            HL_scan_ids, HL_inverse_ids = mair_row_ids_generate((1, 1, H // 2, W // 2), self.scan_len)
            # LH用 Column Scan
            LH_scan_ids, LH_inverse_ids = mair_row_ids_generate((1, 1, H // 2, W // 2), self.scan_len)
            # HH用 Diagonal Scan
            HH_scan_ids, HH_inverse_ids = mair_row_ids_generate((1, 1, H // 2, W // 2), self.scan_len)
            if torch.cuda.is_available():
                HL_scan_ids, HL_inverse_ids = HL_scan_ids.cuda(), HL_inverse_ids.cuda()
                LH_scan_ids, LH_inverse_ids = LH_scan_ids.cuda(), LH_inverse_ids.cuda()
                HH_scan_ids, HH_inverse_ids = HH_scan_ids.cuda(), HH_inverse_ids.cuda()
            # 4. 对高频子带先做patch_embed转token
            for layer in self.layers:
                HL_tokens = layer(HL_tokens, (HL_scan_ids, HL_inverse_ids), (HL.shape[2], HL.shape[3]))
                LH_tokens = layer(LH_tokens, (LH_scan_ids, LH_inverse_ids), (LH.shape[2], LH.shape[3]))
                HH_tokens = layer(HH_tokens, (HH_scan_ids, HH_inverse_ids), (HH.shape[2], HH.shape[3]))
        else:
            for layer in self.layers:
                HL_tokens = layer(HL_tokens, (self.HL_scan_ids, self.HL_inverse_ids), (HL.shape[2], HL.shape[3]))
                LH_tokens = layer(LH_tokens, (self.LH_scan_ids, self.LH_inverse_ids), (LH.shape[2], LH.shape[3]))
                HH_tokens = layer(HH_tokens, (self.HH_scan_ids, self.HH_inverse_ids), (HH.shape[2], HH.shape[3]))
        # x = self.norm(x)  # b seq_len c
        # x = self.patch_unembed(x, x_size)
        LL_tokens = self.patch_embed(LL)
        LL_tokens = self.pos_drop(LL_tokens)
        # for layer in self.layers:
        # LL_tokens = layer(LL_tokens, (self.xs_scan_ids, self.xs_inverse_ids, self.xs_shift_scan_ids, self.xs_shift_inverse_ids), (LL.shape[2], LL.shape[3]))
        LL_tokens = self.norm(LL_tokens)
        LL_feat = self.patch_unembed(LL_tokens, (LL.shape[2], LL.shape[3]))

        # 8. 高频子带经过norm和unembed
        HL_tokens = self.norm(HL_tokens)
        HL_feat = self.patch_unembed(HL_tokens, (HL.shape[2], HL.shape[3]))

        LH_tokens = self.norm(LH_tokens)
        LH_feat = self.patch_unembed(LH_tokens, (LH.shape[2], LH.shape[3]))

        HH_tokens = self.norm(HH_tokens)
        HH_feat = self.patch_unembed(HH_tokens, (HH.shape[2], HH.shape[3]))

        # attention 权重计算
        concat = torch.cat([HL_feat, LH_feat, HH_feat], dim=1)  # → [B, 3C, H, W]
        attn = self.att_conv(concat)  # → [B, 3, H, W]
        weights = F.softmax(attn, dim=1)
        HL_feat, LH_feat, HH_feat = HL_feat * weights[:, 0:1], LH_feat * weights[:, 1:2], HH_feat * weights[:, 2:3]
        # 9. 逆小波变换合成完整特征图
        x_recon = idwt2d(LL_feat, LH_feat, HL_feat, HH_feat, wave='haar')
        return x_recon
...
if __name__ == '__main__':
    ...
    net = MaIR_difscan(img_size=(320, 180), embed_dim=60, d_state=1, ssm_ratio=1.1, dynamic_ids=False, mlp_ratio=1.6,
               upscale=4).cuda()
